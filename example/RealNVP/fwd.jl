...
Data = rand(p, 100000)
data_load = Flux.DataLoader(Data; batchsize=500, shuffle=true)
...
gpu_data_load = gpu(Flux.DataLoader(Data; batchsize=200, shuffle=true))

# Ls_res = [
#     InvertibleAffineBwd() ∘ AffineCouplingRes(d, hdims, 1:2) ∘
#     AffineCouplingRes(d, hdims, 3:4) for i in 1:10
# ]
# ts_res = ∘(Ls_res...)
...
flow_trained, stats, _ = train(flow, data_load, Optimisers.ADAM(2e-4), 100)
...
trans = flow_trained.transform
its = inverse(trans)

tran_big = Flux._paramtype(BigFloat, trans)
its_big = inverse(tran_big)

dat = Data[:, rand(1:10, 10)]
its(dat) .- its_big(dat)
